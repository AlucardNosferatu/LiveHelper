...
def job_ads():
    url = 'https://www.zhipin.com/web/geek/job?city=101230100&page=1'
...
class BossLocalJob(object):
    ...
    def change_page(self, url, total_page):
        start_page_num = int(re.search(r'page=(\d+)', url, re.S).group(1))
        page_group = []
        for i in range(start_page_num, total_page + 1):
            per_link = re.sub(r'page=\d+', 'page=%s' % (i * 1), url, re.S)
            page_group.append(per_link)
        self.page_group = page_group

    def get_all_events(self):
        for html_text in self.html_texts:
            a = re.search('<ul class="job-list-box">(.*?)</ul>', html_text, re.S)
            if a:
                bigger_html = a.group(1)
                events_a = re.findall('<div class="job-card-body clearfix">(.*?)</div>', bigger_html, re.S)
                events_b = re.findall('<div class="job-card-footer clearfix">(.*?)</div>', bigger_html, re.S)
                events = [[a, b] for a, b in zip(events_a, events_b)]
            else:
                events = []
        ...
        for events in self.events:
            for event in events:
                entity = {
                    ...
                    'time': re.search(r'时间：</span>\s+(.*?)\s+<time', event, re.S).group(1),
                    'position': re.search(r'<li title="(.*?)">\s+<span', event, re.S).group(1),
